...
const STORAGE_KEY_E2B = 'neuralchat_e2b_apikey';
...
interface E2BSandboxContextType extends E2BSandboxState {
  setApiKey: (key: string) => void;
  createSandbox: () => Promise<Sandbox | null>;
  createTerminal: (terminalId: string, cols: number, rows: number, onData: (data: Uint8Array) => void) => Promise<any>;
  ...
  disconnectSandbox: () => Promise<void>;
  syncLocalToSandbox: (files: any[], basePath?: string) => Promise<boolean>;
  syncSandboxToLocal: (basePath?: string) => Promise<any[]>;
  onFileChange: (callback: (event: FileChangeEvent) => void) => () => void;
  startFileWatcher: (basePath?: string) => void;
  stopFileWatcher: () => void;
  sandbox: Sandbox | null;
  hasApiKey: boolean;
}

const E2BSandboxContext = createContext<E2BSandboxContextType | null>(null);

export function E2BSandboxProvider({ children }: { children: React.ReactNode }) {
  const [state, setState] = useState<E2BSandboxState>({
    isConnected: false,
    isConnecting: false,
    sandboxId: null,
    error: null,
    apiKey: localStorage.getItem(STORAGE_KEY_E2B) || '',
    isSyncing: false,
  });
  ...
  const setApiKey = useCallback((key: string) => {
    ...
    localStorage.setItem(STORAGE_KEY_E2B, key);
  }, []);
  ...
  const syncLocalToSandbox = useCallback(async (files: any[], basePath = '/home/user') => {
    ...
    try {
      const buildPath = (file: any, allFiles: any[]): string => {
        const parts: string[] = [file.name];
        let current = file;
        
        while (current.parentId && current.parentId !== 'root') {
          const parent = allFiles.find((f: any) => f.id === current.parentId);
          if (parent) {
  ...
  const syncSandboxToLocal = useCallback(async (basePath = '/home/user'): Promise<any[]> => {
    if (!sandboxRef.current) {
      console.error('No sandbox connected');
      return [];
    }
  ...
  const value: E2BSandboxContextType = {
    ...state,
    hasApiKey: !!state.apiKey && state.apiKey.trim().length > 0,
    setApiKey,
    createSandbox,
    createTerminal,
    sendTerminalInput,
  ...
  return (
    <E2BSandboxContext.Provider value={value}>
      {children}
    </E2BSandboxContext.Provider>
  );
}

export function useE2BSandbox() {
  const context = useContext(E2BSandboxContext);
  if (!context) {
    throw new Error('useE2BSandbox must be used within an E2BSandboxProvider');
  }
  return context;
}